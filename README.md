<div align="center">

# Saad Azil — SA3D00N

**Penetration Tester | Network Security | CTF Player**

[![HackTheBox](https://img.shields.io/badge/HackTheBox-SA3D00N-9fef00?style=flat-square\&logo=hackthebox\&logoColor=black)](https://app.hackthebox.com/users/1543873)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-saadazil-0077B5?style=flat-square\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/saadazil/)
[![GitHub](https://img.shields.io/badge/GitHub-SA3D00N-181717?style=flat-square\&logo=github\&logoColor=white)](https://github.com/SaadAzil3)

</div>

---

## About Me

Master's student in **Network Administration & Security** at the **University of Bejaia, Algeria**. I specialize in penetration testing, network defense, vulnerability assessment, and adversary emulation. Currently open to security research collaborations, CTF teams, internships, and offensive security opportunities.

**Education:** Master's in Network Administration & Security (2024–2026)
**Certification:** Certified Network Security Practitioner (CNSP)
**Activity:** Active CTF player on Hack The Box
**Focus Areas:** Penetration Testing, Red Team, Application Security

---

## Experience

| Role                             | Organization           | Location                         | Period              |
| -------------------------------- | ---------------------- | -------------------------------- | ------------------- |
| **Network Security Intern**      | Cevital Agro-industrie | Bejaia, Algeria                          | Mar 2024 – May 2024 |
| **Network & IT Security Intern** | ENAGEO                 | Hassi Messaoud, Ouargla, Algeria | Mar 2026 - Mar 2026         |

### Cevital Agro-industrie

* Simulated enterprise network architectures using GNS3.
* Conducted vulnerability assessments on enterprise networks.
* Performed penetration testing and security validation.
* Delivered comprehensive reports including findings, risk analysis, and mitigation recommendations.

### ENAGEO

* Assisted with enterprise network administration and security operations.
* Worked with security infrastructure and monitoring systems.
* Gained hands-on experience within the energy sector's IT environment.

---

## Featured Projects

| Project                                          | Description                                                                                                   |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| **Intrusion Detection with Nagios**              | Built and monitored an enterprise network in GNS3 with real-time alerting and vulnerability monitoring.       |
| **SMB Relay Attack in Active Directory**         | Demonstrated NTLM relay attacks, credential interception, attack paths, and mitigation techniques.            |
| **Network Hardening & Vulnerability Assessment** | Performed security auditing, vulnerability assessment, and remediation on a simulated enterprise environment. |
| **Enterprise IP Telephony with Asterisk**        | Designed and deployed an IP-based telephony infrastructure using Asterisk.                                    |

---

## Certifications

* Certified Network Security Practitioner (CNSP)

---

## GitHub Statistics

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=SaadAzil3\&show_icons=true\&theme=tokyonight\&hide_border=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=SaadAzil3\&layout=compact\&theme=tokyonight\&hide_border=true)

</div>

---

## Contact

* LinkedIn: https://www.linkedin.com/in/saadazil/
* GitHub: https://github.com/SaadAzil3
* Hack The Box: https://app.hackthebox.com/users/1543873
